...
class MediaController(QtWidgets.QWidget, RegistryBase, LogMixin, RegistryProperties):
    ...
    def _media_state_live(self) -> None:
        ...
        self._tick_player(self.live_controller).update_ui()
    ...
    def _media_state_preview(self) -> None:
        ...
        self._tick_player(self.preview_controller).update_ui()
    ...
    def tick(self, controller) -> None:
        ...
        """
        controller.media_play_item.timer = self._tick_player(controller).get_time()
        self._update_seek_ui(controller)

    @staticmethod
    def _tick_player(controller: SlideController):
        """
        Return the player that should drive timer/seek UI updates.
        """
        if controller.media_play_item.media_type in [MediaType.Audio, MediaType.Dual]:
            return controller.audio_player
        return controller.media_player
