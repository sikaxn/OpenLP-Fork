...
class MediaToolbar(OpenLPToolbar):
    ...
    def _on_action(self, *args):
        ...
        if live_controller and getattr(live_controller, 'mediabar', None) is self:
            controller = live_controller
        elif preview_controller and getattr(preview_controller, 'mediabar', None) is self:
            controller = preview_controller
        else:
            # Fallback for cases where Qt returns a generic QWidget wrapper as the parent.
            widget = self.parentWidget()
            while widget and not hasattr(widget, 'is_live'):
